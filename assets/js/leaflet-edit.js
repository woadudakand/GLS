...
function drawCable(currentCable) {
  ...
  var unassign = currentCable.properties.id.split('*/')[0];
      ...
      return [
        new ol.style.Style({
          ...
          text: new ol.style.Text({
            ...
            scale: 1.3,
            fill: new ol.style.Fill({
              color: unassign === 'un-assign' ? "transparent" : "green",
            }),
            stroke: new ol.style.Stroke({
              color: unassign === 'un-assign' ? "transparent" : "#FFFF99",
              width: 3,
            }),
          }),
        }),
    ...
    insertVertexCondition: (event) => {
      ...
      if (isSelectCalibration) {
        ...
        for (var ind = 0; ind < zoneGeoJson["features"].length; ind++) {
          if (
            zoneGeoJson["features"][ind]["properties"]["id"].indexOf("Cable") ==
            -1
          )
            continue
          ...
          var len = Math.sqrt(
            (tmp[0] - realCoordinate[0]) * (tmp[0] - realCoordinate[0]) +
              (tmp[1] - realCoordinate[1]) * (tmp[1] - realCoordinate[1])
          )
        ...
        layers.forEach((layer) => {
          ...
          if (
            layer.getProperties().name === "section-layer" && layer.getSource().getFeatures()[0] &&
...
function onCalibrationSave() {
  ...
  zoneGeoJson.features.forEach((feature, featureIndex) => {
    console.log(feature.properties.text, selectedFeature.getProperties().layer)
...
function showCalibrationTooltip(feature) {
  ...
  if (selectedFeature) {
    if (selectedFeature.getProperties().value) {
      ...
      var sliced = turf.lineSplit(line, currentpt)["features"].length > 1 ? turf.lineSplit(line, currentpt)["features"][0] : turf.lineSplit(line, turf.point(current))["features"][0];
...
console.log(JSON.stringify({...zoneGeoJson, delId}));
  // * Ajax call to localhost. Address is changeable
...
function init(
  ...
  if (geoJson) {
    ...
    geoJson.features.forEach((feature, key) => {
      ...
      if(feature.properties.calibration) {
        ...
        var option = document.querySelector(`#cable_list option[value^="${feature.properties.id}"]`);
                
          if(option && option.value.indexOf(feature.properties.id) === 0) {
            var startPoint = feature.properties.calibration[0].offset;
            var endPoint = feature.properties.calibration[feature.properties.calibration.length - 1].offset;
            ...
            if(startPoint == opStart && endPoint == opEnd) {
              ...
              feature.properties.id = 'un-assign*/' + feature.properties.id;
  ...
  function initMap() {
    ...
    document.getElementById('map-container').innerHTML = "<div id='leaflet' class='map'></div>";
    leafletMap = new L.Map('leaflet', {
      center: [0, 0],
      zoom: 0,
      layers: [
