        ...
        for (let j = 0; j < alertList.length; j++) {
            if (j === 0) {
                ...
                if (alertList[j] < 0) {
                    ...
                    alertValList.push(
                        `${cableInfo[0]}#${cableInfo[1]}#${alertList[j]}`
                    );
                }
            }
        }
        ...
        if (cableLength != undefined) {
            ...
            zoneGeoJson.features.forEach((feature, featureIndex) => {
                if (
                    ...
                    feature.properties.id === sel_cable
                ) {
                    cableInfo =
                        zoneGeoJson.features[featureIndex].properties
                            .calibration;
                }
            });
...
$(document).on('click', '#btn_add_cable', null, function () {
    ...
    const removeId = selectedFeature.getProperties().id;
    ...
    if (
        selectedFeature.getGeometry().getType() !== 'LineString' &&
        ...
        var isCalibration =
            ...
            calibrationPoints[selectedFeature.values_.id] &&
            calibrationPoints[selectedFeature.values_.id].length;
        if (isCalibration) {
        ...
        document.querySelectorAll(`#cable_list option`).forEach((option) => {
            ...
            if (
                option.value.indexOf(cableId) === 0 &&
                option.innerHTML === currentCable.properties.text
            ) {
...
$(document).on('click', '#btnDelete', null, function () {
    ...
    if (Object.values(selectedFeature).length) {
        ...
        if (isSelected && isSelected.properties && selectedZone.text) {
            delId = [...delId, isSelected.properties.id];
            removeLayersFromMap(getSelectedLayer(isSelected.properties));
        } else {
            if (selectedFeature && selectedFeature.getProperties().layer) {
                delId = [...delId, selectedFeature.getProperties().id];
                removeDrawingFromLayer(
                    selectedFeature,
                    getLayer(selectedFeature)
                );
        ...
        document.querySelectorAll(`#cable_list option`).forEach((option) => {
            if (option.value.indexOf(selectedZone.id) === 0) {
                option.classList.remove('active');
            }
        });
    } else {
...
function drawCable(currentCable) {
    ...
    var coordinates =
        currentCable?.geometry?.coordinates ||
    ...
    var lineStringVector = new ol.layer.Vector({
        ...
        style: function (f) {
            ...
            return [
                new ol.style.Style({
                    stroke: new ol.style.Stroke({
                        color: currentCable?.properties?.color || '#557ef8',
                        width: 16,
                    }),
        ...
        insertVertexCondition: (event) => {
            ...
            if (isSelectCalibration) {
                ...
                for (var ind = 0; ind < zoneGeoJson['features'].length; ind++) {
                    if (
                        zoneGeoJson['features'][ind]['properties'][
                            'id'
                        ].indexOf('Cable') == -1
                    )
                        continue;
                    let lines = new ol.geom.LineString(
                        zoneGeoJson['features'][ind]['geometry']['coordinates']
                    );
                    let tmp = lines.getClosestPoint([
                        realCoordinate[0],
                        realCoordinate[1],
                    ]);
                    var len = Math.sqrt(
                        (tmp[0] - realCoordinate[0]) *
                            (tmp[0] - realCoordinate[0]) +
                            (tmp[1] - realCoordinate[1]) *
                                (tmp[1] - realCoordinate[1])
                    );
                    if (
                        typeof closestLength == 'undefined' ||
                        len < closestLength
                    ) {
                        closestLength = len;
                        closest = tmp;
                ...
                features
                    ...
                    .forEach((item, index) => {
                        ...
                        return (zoneGeoJson.features[
                            featureIndex
                        ].properties.calibration[index].coordinates = item);
                    });
            }
        });
    });
            ...
            for (let i = 2; i < map.getLayers().getLength(); i++) {
                ...
                for (const feature of features) {
                    if (
                        ...
                        zoneGeoJson.features.map(
                            (zoneFeature, featureIndex) => {
...
function onCalibrationDelete() {
    ...
    zoneGeoJson.features.forEach((feature, featureIndex) => {
        if (feature.properties.id === selectedFeature.getProperties().layer) {
            ...
            calibrationPoints[feature.properties.id].shift();
...
function showCalibrationTooltip(feature) {
    ...
    if (selectedFeature) {
        ...
        if (selectedFeature.getProperties().value) {
            ...
            for (let i = data.length - 1; i > 0; i--) {
                for (let j = 0; j < data.length; j++) {
                    if (i !== j && data[i][1] === data[j][1]) {
                        // Step 2: Add 100 to the second item
                        data[i][1] += 0.10001;
                    }
            ...
            document.querySelectorAll(`#cable_list option`).forEach((el) => {
                if (el.value.indexOf(selectedFeature.values_.layer) === 0) {
                    cableElemVal = el.value;
                }
            });
...
$(document).on('click', '#btnChangeColor', null, function () {
    ...
    // targetCable.setStyle(null); // Clear existing style
    if (targetCable) {
        const color = targetCable.getProperties().color || '#ff00ff';

        // targetCable.setStyle(null); // Reset
        targetCable.setStyle(
            new ol.style.Style({
                stroke: new ol.style.Stroke({
                    color: color,
                    width: 16,
...
function nudgeIdenticalCoordinates(coords) {
    for (let i = 1; i < coords.length; i++) {
        const [prevX, prevY] = coords[i - 1];
        let [curX, curY] = coords[i];

        // If consecutive X values are identical, nudge the current X
        if (prevX === curX) {
            curX += 0.00001;
        ...
        // If consecutive Y values are identical, nudge the current Y
        if (prevY === curY) {
            curY += 0.00001;
            ...
            if (features.length) {
                features.forEach((feature) => {
                    ...
                    if (
                        ...
                        featureCoordinates =
                            nudgeIdenticalCoordinates(featureCoordinates);
                    }
...
$(document).on('click', '#btnSave', null, function (e) {
    ...
    var urlss = 'http://' + ipAddress + ':8080/GLS/updatejson.html';

    // * Ajax call to localhost. Address is changeable
    $.post(urlss, {
        //$.post("http://18.215.137.140:8080/GLS//updatejson.html", {
        data: JSON.stringify({
            ...zoneGeoJson,
            ...zoneGeoJson,
            delId,
            ...
            'All Cable Id': `${
                usedCables === '' ? 'No selected Zone' : usedCables
            }`,
...
async function init(
    ...
    if (geoJson) {
        delId = geoJson.delId ? geoJson.delId : [];
        let drawingList = [];
        geoJson.features.forEach((feature, key) => {
            const newCalArray = [];
            if (feature.properties.calibration) {
                ...
                var option = null;
                document
                    .querySelectorAll(`#cable_list option`)
                    .forEach((el) => {
                        if (el.value.split('/')[0] == feature.properties.id) {
                            option = el;
                        }
                    });

                if (option) {
                    var startPoint = feature.properties.calibration[0].offset;
                    var endPoint =
                        feature.properties.calibration[
                            feature.properties.calibration.length - 1
                        ].offset;
                    var opStart = option.value.split('/')[1].split('-')[0];
                    var opEnd = option.value.split('/')[1].split('-')[1];
                    if (startPoint == opStart && endPoint == opEnd) {
                        option.classList.add('active');
                    } else {
                        feature.properties.text = ' ' + key;
                        feature.properties.id =
                            'un-assign*/' + feature.properties.id;
                    }
                } else {
                    feature.properties.text = ' ' + key;
                    feature.properties.id =
    ...
    async function initMap() {
        ...
        if (zoomValue < 1) {
            zoomValue =
                (viewerHeight / realHeight) *
                (1 + (realHeight / viewerHeight) ** 2);
        }
            ...
            style: (f) => {
                return new ol.style.Style({
                    image: new ol.style.Circle({
                        ...
                        fill: new ol.style.Fill({
                            color: (f.get('color') || [255, 128, 0]).concat([
                                0.3,
                            ]),
                        }),
                    }),
    ...
    map.on('pointermove', function (evt) {
        ...
        var hit = this.forEachFeatureAtPixel(evt.pixel, (feature, layer) => {
            ...
            if (
                ...
                selectedZone.features[0].getProperties().id.indexOf('Cable') >
                    -1
            ) {
        ...
        if (hit && calibrations.length) {
            ...
            calibrations.map((element, elementIndex) => {
                ...
                element.coordinates.map((coordinateItem, index) => {
                    if (
                        coordinateItem.toFixed(0) !==
                        coordinates[index].toFixed(0)
                    ) {
                        isSame = false;
                    }
                });
...
document.addEventListener(
    ...
    function (event) {
        ...
        if (event.detail > 0 && event.button == 0) {
            ...
            event.stopPropagation();
            $('body').addClass('mouseDisable');
            setTimeout(() => {
                if (event.detail < 2) {
                    $('body').removeClass('mouseDisable');
                } else {
                    $('body').addClass('mouseDisable');
                }
            }, 200);
