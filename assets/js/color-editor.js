...
function drawCircle(currentCable) {
    initSelectedZone();
    var coordinates = currentCable.geometry.coordinates;
    var radius = currentCable.geometry.radius;
...
function addSelectAction() {
    ...
    actionOnMap.on('select', function (event) {
        ...
        event.deselected.forEach((each) => {
            ...
            console.log(each.getProperties());
            if (each.getGeometry().getType() === 'Circle') {
                console.log(each);
                // var $cableElem = $("#cable_list");
                // var cableId = $cableElem.val().split("/")[0]
                // var currentCable = {
...
function adjustCoordinatesIfNeeded(x1, y1, x2, y2) {
    if (x1 === x2) {
        x2 += 0.000001;
    }
    if (y1 === y2) {
...
/**
 * Ensures no two consecutive points have the exact same X or Y
 * by nudging the current coordinate by 0.00001 if needed.
 */
...
function saveFunc(e) {
    ...
    var urlss = 'http://' + ipAddress + ':8080/GLS/json.html';
    //  alert("urlss="+urlss)
    $.post(urlss, {
        data: JSON.stringify({
            ...zoneGeoJson,
...
function init(
    ...
    function ip() {
        ...
        ////   if (form.usermail.value == "admin@gmail.com" && form.pass.value == "123456") {
        //       return true;
        //    }
        //  else {
